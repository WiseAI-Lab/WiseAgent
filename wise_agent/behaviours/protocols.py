...
from wise_agent.acl import ACLMessage, Filter
from wise_agent.misc.utility import print_progress_bar
...
class FipaRequestProtocol(FipaProtocol):
    ...
    def on_start(self):
        ...
        if self.is_initiator and self.message != None:
            memory_piece = self.agent.memory_handler.generate_memory_from_message(
                message=self.message,
                category=self.__class__.__name__
            )

            self.agent.send(memory_piece)
